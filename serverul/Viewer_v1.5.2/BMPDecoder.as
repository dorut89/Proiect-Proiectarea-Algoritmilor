...
package  {
	...
	public class BMPDecoder {
		...
		public function decode( data:ByteArray ):BitmapData {
			...
			bd = new BitmapData( nWidth, nHeight,true );
			...
			try {
				for ( y = nHeight - 1; y >= 0; --y ){
					...
					for ( x = 0; x < nWidth; x += 8 ){
						...
						for ( i = 0; i < 8; ++i ){
							cul = palette[ col >> ( 7 - i ) & 0x01 ];
							if(cul == 0){								
								bd.setPixel32( x + i, y, 0xFFFFFFFF);
							}else{
								bd.setPixel32( x + i, y, 0x00000000);
							}
						}
					}
				}
